...
void ft_on_object_ticked(t_obj *obj, unsigned long tick, void *custom_data)
{
    ...
    if (typeId == UNIT_WARRIOR)
    {
        ...
        if (nearestOpponent)
            ft_travel_to_pos(unit, nearestOpponent->pos);
        else
            ft_travel_to_pos(unit, ft_get_nearest_opponent_core(unit)->pos);
    }
    else if (typeId == UNIT_MINER)
    {
        t_obj *nearestResource = ft_get_nearest_resource(unit);
        if (nearestResource)
            ft_travel_to_pos(unit, nearestResource->pos);
        else
            ft_travel_to_pos(unit, ft_get_nearest_opponent_core(unit)->pos);
    }
    else if (typeId == UNIT_CARRIER)
    {
        ...
        if (unit->s_unit.balance <= 0 && closestUnitWithMoney != NULL)
            ft_travel_to_pos(unit, closestUnitWithMoney->pos);
        else
            ft_travel_to_pos(unit, ft_get_my_core()->pos);
    }
    else if (typeId == UNIT_BOMBERMAN)
    {
